...
class FilialaiController extends AbstractController
{
    ...
    public function delete($filialasID)
    {
        ...
        if ($filialas != null)
        {
	        ...
	        $visi_auto = $this->getDoctrine()->getRepository(TransportoPriemone::class)->findBy(['filialas' => $filialasID]);
			foreach($visi_auto as $auto)
			{
				$automobilis = $entityManager->getRepository(TransportoPriemone::class)->find($auto->getId());
				$automobilis->setFilialas(null);
				$entityManager->flush();
			}
